...
begin 
	...
	<p style="padding-bottom:1cm"> </p>
	<p align=center style="font-size:25px; font-family:family:Georgia"> <b> Assignment 07 </b> <p>
	<p style="padding-bottom:1cm"> </p>
...
1. Select 10 stocks from the CRSP dataset and calculate monthly returns.
2. Estimate the expected returns of the 10 stocks as the historical average monthly return of the 10 stocks over the 2000 to 2023 period. Use the same data to estimate the covariance matrix of returns.
3. Given the estimate of μ and σ from part 2., plot the mean variance frontier of risky assets. Also show where the individual assets are located relative to the mean-variance frontier of risky assets.
4. Plot the mean-variance frontier with a riskfree asset.
5. Show visually where the tangency portfolio is located on the mean-variance frontier of risky assets, and calculate i) the portfolio weights, ii) the expected return, and iii) the return standard deviation of the tangency portfolio.
6. Suppose that short-selling is prohibited. Set up the mean-variance optimization problem with a short-sales constraint (all portfolio weights are required to be non-negative) using the Julia libraries `Convex.jl` and `SCS.jl`. Visualize the impact of short-sales constraints by plotting the mean-variance frontier with and without short-sales constraints.
"""
...
manifest_format = "2.0"
project_hash = "9fc3c4a6870872d4c20b8dcb757241997c40b7ef"
